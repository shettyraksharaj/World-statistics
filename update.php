...
/*-----SQL query to fetch data from the database with matching con_id------*/
...
/*-----------check if user clicked on update button-------*/
if (isset($_POST['update'])) {
    ...
    if ($flaguperr == 0) { //checking for errors
      if ($flagsize < 1024000) { //checking for file size
        if ($imgext == "svg") { //checking for file format
          if (move_uploaded_file($flaginiloc, $imgNloc)) { //moving the file to a flag folder and relpacing the existing file
          } else {
  ...
  header('location:admindataview.php'); //Reroute to main page
  return;
}
?>
<!---HTML--->
...
/*------Javascript to show alertbox-------*/
    ...
    /*---Javascript to she preview of an image---*/
    $(document).ready(() => {
